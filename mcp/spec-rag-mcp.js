...
import { z } from "zod";
import { buildContextCacheKey, readContextCache, writeContextCache, } from "../scripts/context-cache.js";
...
async function readFeatureContext(feature) {
    const [context, traceabilitySummary] = await Promise.all([
        ...
        readFile(path.resolve("specs", "features", feature, "TRACEABILITY-SUMMARY.md"), "utf8").catch(() => null),
    ]);
...
function rankEntries(entries, feature, version) {
    return entries
...
function buildSummary(entry, content) {
    const sections = getSections(content);
    const preferredHeadings = entry.type === "feature"
        ? [
            "Objective",
            "Scope",
            "Out of scope",
    ...
    const preferred = sections.filter((section) => preferredHeadings.some((heading) => heading.toLowerCase() === section.heading.toLowerCase()));
...
async function retrieveRelevantSpecs(feature, version, detail = "summary") {
    ...
    const relatedIds = new Set([featureSpec.id, ...(featureSpec.dependsOn ?? [])]);
    ...
    const documents = await Promise.all(relatedEntries.map(async (entry) => ({
        entry,
        content: await readSpecFile(entry.path),
    })));
    const payload = documents
        .map(({ entry, content }) => `# ${entry.id}
Path: ${entry.path}
Type: ${entry.type}
...
${detail === "full" ? content : buildSummary(entry, content)}`)
    ...
    if (detail === "full" && estimatedTokens > FULL_BUDGET_HARD_BLOCK) {
        throw new Error(`Full mode hard block: estimated ${estimatedTokens} tokens exceeds the ${FULL_BUDGET_HARD_BLOCK}-token limit. Use chunked or summary mode instead.`);
    }
    if (!featureContext) {
...
server.registerTool("retrieve_relevant_specs", {
