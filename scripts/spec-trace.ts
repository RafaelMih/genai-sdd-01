...
function parseVersionFromName(name: string): VersionTuple | null {
  const flat = name.match(/^spec-v(\d+)\.(\d+)\.(\d+)\.md$/i);
  if (flat) return [Number(flat[1]), Number(flat[2]), Number(flat[3])];

  const nested = name.match(/^v(\d+)\.(\d+)\.(\d+)$/i);
  if (nested) return [Number(nested[1]), Number(nested[2]), Number(nested[3])];
...
async function findLatestSpecPath(
  ...
  const candidates: Array<{ version: VersionTuple; fullPath: string }> = [];
...
function extractSpecReference(traceabilityText: string): string | null {
...
function getMappingLines(traceabilityText: string): string[] {
...
async function main(): Promise<void> {
  ...
  for (const entry of features) {
    ...
    if (!(await exists(traceabilityPath))) {
      console.error(
        `[spec:trace] Missing TRACEABILITY.md for feature "${featureName}" at "${traceabilityPath}"`,
      );
      hasErrors = true;
      continue;
    }
    ...
    if (!specReference) {
      ...
      if (!isValidSpecReferenceFormat(specReference)) {
        console.error(
          `[spec:trace] Invalid spec reference format in "${traceabilityPath}": ${specReference}`,
        );
        hasErrors = true;
      } else {
        ...
        if (!(await exists(normalized))) {
          ...
          if (!latestSpec) {
            console.error(
              `[spec:trace] Referenced spec does not exist and no spec versions were found for feature "${featureName}": ${specReference}`,
            );
          } else {
            console.error(
              `[spec:trace] Referenced spec does not exist: ${specReference}. Latest available spec is "${latestSpec}"`,
            );
          }
          hasErrors = true;
        }
    ...
    if (hasTableRows && !hasTraceabilityTableHeader(traceabilityText)) {
      console.error(
        `[spec:trace] Traceability table header is invalid or missing in "${traceabilityPath}"`,
      );
      hasErrors = true;
    }
  }
