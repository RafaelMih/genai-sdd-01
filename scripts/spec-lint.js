...
function lintAcceptanceCriteria(acText, issues) {
    ...
    lines.forEach((line, index) => {
        ...
        if (!/\b(must|shall|should|shows|redirects|returns|blocks|displays|creates|rejects|allows|prevents|disables)\b/i.test(item)) {
            pushIssue(issues, "WARN", `AC-${index + 1}: may not define observable behavior clearly: "${item}"`);
        }
        if (!/[.?!]$/.test(item)) {
...
function lintAuthSpec(text, sections, issues) {
    ...
    const hasRedirectPath = /redirect(path)?\s*:\s*["'`]?\/[a-z0-9/_-]+["'`]?/i.test(text) ||
        /redirects?\s+to\s+["'`]?\/[a-z0-9/_-]+["'`]?/i.test(text);
    const hasProvisioning = /sign-up|signup|pre-seeded|pre seeded|admin-created|admin created|account provisioning|account creation|accounts are created/i.test(text);
    const hasErrorContract = /auth\/[a-z-]+|error code|error mapping|invalid email or password|network-request-failed|wrong-password|user-not-found|friendly auth error/i.test(text);
    const hasAlreadyAuthenticatedContract = /already-authenticated|already authenticated|if authenticated user visits \/login|signed-in user navigates to \/login|redirect immediately/i.test(text);
    const hasLoadingState = /loading state|pending state|spinner|button disabled|disable submit during request|submitting/i.test(text);
    const mentionsSecurityRules = /security-rules|security rules/i.test(text);
    const explainsFirestoreInteraction = /firestore|document creation|users\/\{uid\}|users\/\{uid\}|create.*user document|post-login write|post login write/i.test(text);
    const hasOpenRedirectDecision = /dashboard\s+path\s+is\s+undecided|\/dashboard\s+vs\s+\/app|open question/i.test(fullText);
    if (!hasRedirectPath) {
    ...
    if (!hasAlreadyAuthenticatedContract) {
        pushIssue(issues, "BLOCKER", "[auth] Missing behavior for already-authenticated users visiting /login.");
    }
    if (!hasLoadingState) {
    ...
    if (mentionsSecurityRules && !explainsFirestoreInteraction) {
        pushIssue(issues, "WARN", "[auth] Mentions security rules without explaining Auth ↔ Firestore interaction.");
    }
    if (hasOpenRedirectDecision) {
        pushIssue(issues, "BLOCKER", "[auth] Contains unresolved redirect path decision.");
    }
    if (/friendly auth error/i.test(acText) &&
        !/invalid email or password|mapped error|error code/i.test(acText)) {
        pushIssue(issues, "BLOCKER", '[auth] Acceptance criteria mention "friendly auth error" without a concrete message or mapping.');
    }
}
function lintFile(file) {
    ...
    const isAuthLike = /[\\/]auth[\\/]/i.test(file) ||
