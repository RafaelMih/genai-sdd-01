...
async function saveManifest(entries: SpecManifestEntry[]): Promise<void> {
  await writeFile(MANIFEST_PATH, `${JSON.stringify(entries, null, 2)}\n`, "utf8");
}

async function appendArchivePointer(
  featureDir: string,
  feature: string,
  archivedFiles: string[],
): Promise<void> {
  const changelogPath = path.join(featureDir, "changelog.md");
  if (!(await pathExists(changelogPath)) || archivedFiles.length === 0) return;

  const changelog = await readFile(changelogPath, "utf8");
  const pointer = `\n## Archived specs\n- Historical specs moved to \`specs/archive/${feature}/\`: ${archivedFiles.join(", ")}\n`;

  if (changelog.includes("## Archived specs")) return;

  await writeFile(changelogPath, `${changelog.trimEnd()}\n${pointer}`, "utf8");
}

async function archiveFeatureSpecs(
  feature: string,
  ...
  if (specFiles.length <= 1) return [];

  const activeFile = specFiles[specFiles.length - 1];
  const featureManifestEntries = manifest.filter(
    (entry) => entry.type === "feature" && entry.feature === feature,
  );
  ...
  for (const fileName of specFiles) {
    ...
    for (const manifestEntry of featureManifestEntries) {
      if (path.basename(manifestEntry.path) !== fileName) continue;
...
async function main(): Promise<void> {
  ...
  const summary: Array<{ feature: string; archived: string[] }> = [];
