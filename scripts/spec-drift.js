...
function getLatestFeatureSpecs() {
    if (!fs.existsSync(FEATURES_SPEC_ROOT))
        return [];
    const specs = [];
...
function extractDependencyPath(specText) {
    const match = specText.match(/specs\/technical\/firestore-schema-v\d+\.\d+\.\d+\.md/i);
    return match?.[0] ?? null;
}
function extractFirestoreContractFields(specText) {
    const contractMatch = specText.match(/## Firestore write contract([\s\S]*?)(?:\n## |\r\n## |\n# |\r\n# |$)/i);
    if (!contractMatch)
        return [];
    const ids = new Set();
...
function extractUsersSchemaFields(schemaText) {
    const usersSectionMatch = schemaText.match(/### users\/\{uid\}([\s\S]*?)(?:\n### |\r\n### |\n## |\r\n## |$)/i);
    if (!usersSectionMatch)
        return [];
    const ids = new Set();
...
for (const spec of getLatestFeatureSpecs()) {
    ...
    if (missingFields.length > 0) {
        console.error(`[spec:drift] ${spec.feature}: Firestore write contract has fields missing from schema ${schemaDependency}: ${missingFields.join(", ")}`);
        hasErrors = true;
    }
}
if (hasErrors) {
