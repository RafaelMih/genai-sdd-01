...
async function loadManifest() {
    ...
    return JSON.parse(raw).filter((entry) => entry.status !== "archived");
}
async function loadFeatureContext(featureName) {
    const [context, traceabilitySummary] = await Promise.all([
        ...
        readFile(path.resolve("specs", "features", featureName, "TRACEABILITY-SUMMARY.md"), "utf8").catch(() => null),
    ]);
    if (!context && !traceabilitySummary)
        return null;
...
function resolveTargetSpec(manifest, featureName, targetVersion) {
...
function scoreChunk(chunk, featureName, relatedSpecIds, targetVersion) {
    let score = 0;
    if (!relatedSpecIds.has(chunk.specId) && chunk.feature !== featureName) {
        return -1_000;
    }
    if (chunk.feature === featureName)
        score += 100;
    if (relatedSpecIds.has(chunk.specId))
        score += 60;
    if (targetVersion && chunk.version === targetVersion)
        score += 40;
    if (targetVersion && chunk.version !== targetVersion)
        score -= 100;
    if (chunk.type === "feature")
        score += 25;
    if (chunk.type === "technical")
        score += 10;
    if (/objective/i.test(chunk.section))
        score += 20;
    if (/scope/i.test(chunk.section))
        score += 20;
    if (/out of scope/i.test(chunk.section))
        score += 15;
    if (/acceptance criteria/i.test(chunk.section))
        score += 45;
    if (/contracts?/i.test(chunk.section))
        score += 35;
    if (/dependencies/i.test(chunk.section))
        score += 25;
    if (/open questions/i.test(chunk.section))
        score += 25;
    if (/user flow/i.test(chunk.section))
        score += 20;
    if (/tests?/i.test(chunk.section))
        score += 10;
...
async function main() {
    ...
    if (!targetSpec) {
        throw new Error(`No feature spec manifest entry found for "${feature}"${version ? ` v${version}` : ""}.`);
    }
    const relatedSpecIds = new Set([targetSpec.id, ...(targetSpec.dependsOn ?? [])]);
    ...
    for (const chunk of rankedChunks) {
        if (relevantChunks.length >= MAX_CONTEXT_CHUNKS)
            break;
        if (relevantChunks.length > 0 && totalTokens + chunk.tokens > MAX_CONTEXT_TOKENS)
            continue;
        relevantChunks.push(chunk);
        totalTokens += chunk.tokens;
    }
    if (relevantChunks.length === 0) {
        throw new Error(`No relevant spec chunks found for "${feature}"${version ? ` v${version}` : ""}. Run npm run index:specs or check your specs.`);
    }
    ...
    const budgetWarning = budgetExceeded
        ...
        : "";
    const outputTokens = Math.ceil(`${summaryPrefix}${context}`.split(/\s+/).filter(Boolean).length * 1.3);
    if (outputTokens > FULL_BUDGET_HARD_BLOCK) {
        throw new Error(`Context hard block: estimated ${outputTokens} tokens exceeds the ${FULL_BUDGET_HARD_BLOCK}-token limit. Reduce chunk count or use a more specific query.`);
    }
    const output = `
