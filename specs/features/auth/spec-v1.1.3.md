...
- AC9: Firebase returns any other error code →
  ...
  and the submit button
- AC10: On successful sign-in, the submit handler in `LoginForm` calls
  navigate('/dashboard', { replace: true }) directly (not via `onAuthStateChanged`);
...
- AC11: After submit (validation passed), the submit button is disabled and shows "Entrando…"
  ...
  the Firebase error message is shown; on success the page on success the redirect defined in AC10 is executed
- AC12: A user already authenticated (detected via `onAuthStateChanged` on page mount) who opens /login
